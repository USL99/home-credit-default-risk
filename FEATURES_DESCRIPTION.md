# Home Credit Default Risk — описание признаков
...
Качество оценивается с помощью ROC-AUC.

## 2. Связь таблиц

`application_train` / `application_test` — файлы данных для обучения и тестирования модели.
...
## 3. Как читать название признака
...
- `LAST` — последнее, то есть новое значение.
...
Специальное значение `365243` в `DAYS_EMPLOYED` не является реальным стажем. Для KMeans оно заменяется на `NaN`, чтобы не искажать расстояния.
...
- `ENDDATE_DIFF = DAYS_ENDDATE_FACT - DAYS_CREDIT_ENDDATE` — показывает, насколько фактическая дата закрытия отличается от плана.
...
- `BUREAU_LAST_3_<КОЛОНКА>_*` / `BUREAU_LAST_5_<КОЛОНКА>_*` / `BUREAU_LAST_10_<КОЛОНКА>_*` — признаки по последним 3, 5 и 10 кредитам.
- `BUREAU_LAST_<КОЛОНКА>` — значение по самому новому кредиту.
- `BUREAU_<КАТЕГОРИЯ>_COUNT` / `BUREAU_<КАТЕГОРИЯ>_SHARE` — количество и доля каждой категории.
- `BUREAU_TREND_3_10_<КОЛОНКА>` — медиана последних 3 кредитов минус медиана последних 10.
- `BUREAU_TREND_5_10_<КОЛОНКА>` — медиана последних 5 кредитов минус медиана последних 10.
...
`BB_STATUS_<СТАТУС>_COUNT` / `BB_STATUS_<СТАТУС>_SHARE` — количество и доля каждого статуса.
...
- `PREV_CREDIT_TO_APPLICATION_MEDIAN` / `PREV_CREDIT_TO_APPLICATION_MAX` — Типичная и максимальная доля одобренной суммы.
...
`PREV_<КАТЕГОРИЯ>_COUNT` / `PREV_<КАТЕГОРИЯ>_SHARE` — Количество и доля каждой категории.
...
- `POS_COMPLETION_RATIO_MEDIAN` / `POS_COMPLETION_RATIO_MAX` — Типичная и максимальная пройденная доля графика.

- `POS_CONTRACT_WITH_HISTORY_COUNT` — Сколько договоров с историей есть у клиента.

- `POS_HISTORY_LENGTH_MAX` / `POS_HISTORY_LENGTH_MEDIAN` — Максимальная и типичная длина истории договоров.
...
`POS_NAME_CONTRACT_STATUS_<СТАТУС>_COUNT` / `POS_NAME_CONTRACT_STATUS_<СТАТУС>_SHARE` — Количество и доля каждого состояния договора.
...
- `INST_PAYMENT_MISSING_COUNT` / `INST_PAYMENT_MISSING_SHARE` — количество и доля платежей без фактической даты.
...
- `INST_LATE_PAYMENT_COUNT` / `INST_LATE_PAYMENT_SHARE` — количество и доля просроченных платежей.

- `INST_PAYMENT_DIFF_MEDIAN` / `INST_PAYMENT_DIFF_MAX` — Типичная и максимальная недоплата.

- `INST_UNDERPAID_COUNT` / `INST_UNDERPAID_SHARE` — количество и доля недоплаченных платежей.

- `INST_PAYMENT_RATIO_MEDIAN` / `INST_PAYMENT_RATIO_MIN` — Типичное и минимальное отношение оплаты к требуемой сумме.
...
- `CC_UTILIZATION_MEDIAN` / `CC_UTILIZATION_MAX` — типичная и максимальная доля использованного лимита.

- `CC_PAYMENT_TO_MIN_MEDIAN` / `CC_PAYMENT_TO_MIN_MAX` — типичное и максимальное отношение платежа к минимуму.

- `CC_AVG_DRAWING_MEDIAN` / `CC_AVG_DRAWING_MAX` — типичный и максимальный средний размер операции.
...
- `CC_NAME_CONTRACT_STATUS_<СТАТУС>_COUNT` / `CC_NAME_CONTRACT_STATUS_<СТАТУС>_SHARE` — количество и доля каждого состояния договора.

- `CC_LAST_3M_* / CC_LAST_6M_* / CC_LAST_12M_*` — признаки за последние 3, 6 и 12 месяцев.

## 12. Временные trend-признаки
...
BUREAU_LAST_3_AMT_CREDIT_SUM_median - BUREAU_LAST_10_AMT_CREDIT_SUM_median
```
...
### `previous_application`: trend-признаки
...
### `POS_CASH_balance`: trend-признаки
...
### `installments_payments`: trend-признаки
...
### `credit_card_balance`: trend-признаки
...
Лучший результат для основной задачи показал `k = 3`, поэтому в финальной версии используется KMeans с тремя кластерами.

### Финальные KMeans-признаки

- `KMEANS_3_CLUSTER` — номер ближайшего кластера клиента. В LightGBM передаётся как категориальный признак; значения `0`, `1` и `2` не имеют числового порядка.

- `KMEANS_3_DISTANCE_0` — евклидово расстояние клиента до центра кластера 0.

- `KMEANS_3_DISTANCE_1` — евклидово расстояние клиента до центра кластера 1.

- `KMEANS_3_DISTANCE_2` — евклидово расстояние клиента до центра кластера 2.
...
- `SK_ID_CURR` — технический идентификатор клиента.
